...
class Ray:
    ...
    def __init__(self, x0=0, y0=0, kx0=0, ky0=1, freq=135e9, amp=1, n_field=None):
        ...
        # wave vector and frequency processing
        # from normalized wave vector to physical
        # from physical wavee vector get frequency
        ...
        if not (n_field is None):
            ...
            if Ray.traceType == 'c':
                ...
                i = trace(self.x, self.y, self.kx, self.ky,  # changeable access by pointer
                          st, dx, dy,  # fixed used as static parameters
                          n_field.nx, n_field.ny,  # fixed used as parameters
                          n_field.ax, n_field.ay)  # access by pointer
            else:
                i = 0
                while i < Ray.MaxStep and 0 <= self.x[i] < n_field.nx - 1 and 0 <= self.y[i] < n_field.ny - 1:
                    x_ind = int(np.trunc(self.x[i]))
                    y_ind = int(np.trunc(self.y[i]))

                    self.x[i + 1] = self.x[i] + (self.kx[i] * st + n_field.ax[y_ind, x_ind] * st ** 2 / 2) / dx
                    self.y[i + 1] = self.y[i] + (self.ky[i] * st + n_field.ay[y_ind, x_ind] * st ** 2 / 2) / dy
